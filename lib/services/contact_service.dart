...
import 'package:opole/service/reel_service.dart';
...
class _ViewContactState extends State<ViewContact> {
  ...
  Future<void> _validateAccess() async {
    ...
    try {
      ...
      // Obtener datos del usuario dueÃ±o para verificar privacidad
      final ownerDoc = await FirebaseFirestore.instance
          .collection('users')
          .doc(reelData['userId'])
          .get();
      
      final ownerData = ownerDoc;
      
      // Verificar privacidad del dueÃ±o
      final showPhone = ownerData?['showPhone'] ?? true;
      final showEmail = ownerData?['showEmail'] ?? true;
      
      // Si el dueÃ±o NO quiere mostrar nada, bloquear
      if (!showPhone && !showEmail) {
        setState(() {
          _isLoading = false;
          _errorMessage = "El usuario no comparte informaciÃ³n de contacto";
        });
        return;
      }

      // Verificar si el contacto fue compartido con el usuario actual
      final contactSharedWith = reelData['contactSharedWith'] as String?;
      
      if (contactSharedWith != session.uid) {
        setState(() {
          _isLoading = false;
          _hasAccess = false;
      ...
      // Verificar que exista informaciÃ³n de contacto respetando privacidad
      final hasPhone = showPhone && reelData['ownerPhone'] != null;
      final hasEmail = showEmail && reelData['ownerEmail'] != null;
  ...
  Widget _buildContactInfo() {
    ...
    // Obtener preferencias de privacidad del dueÃ±o
    final showPhone = _reelData?['ownerShowPhone'] ?? true;
    final showEmail = _reelData?['ownerShowEmail'] ?? true;
    
    final hasPhone = showPhone && contact['phone'] != 'No disponible';
    final hasEmail = showEmail && contact['email'] != 'No disponible';
    final hasWhatsapp = hasPhone && contact['whatsapp'] != 'https://wa.me/';
